module Main exposing (AppModel, Msg(..), bitIntro, combinatricsNotation, corruptTheBit, errorProbability, initialModel, main, oddEven, packetIntro, parityIntro, pluralString, se, singleErrorCorrection, singleParity, transmissionEfficiency, transmittedBits, tutorialConclusionText, tutorialFooterText, tutorialIntroText, update, view)
...
singleParity model =
    case modBy 2 (model.packetModel |> Packet.packetValue |> List.map String.fromInt |> concat |> split "0" |> join "" |> length) of
        0 ->
            model.oddeven

        1 ->
            modBy 2 (model.oddeven + 1)
    ...
    Html.div
        ...
        , Html.p
            ...
            , Html.input
                [ HA.type_ "number"
                , HA.size 50
                , HA.value (String.fromFloat model.bitProbability)
                , HA.max "1"
                , HA.min "0"
                , HA.step "0.01"
            ...
            , Html.span []
                ...
                , Html.text " * (1 - "
                , Html.text (String.fromFloat model.bitProbability ++ ")")
                , Html.sup [] [ Html.text (String.fromInt n) ]
                , Html.text " = "
                , Html.text
                    (String.fromFloat
            ...
            , Html.span []
                ...
                , Html.text " * (1 - "
                , Html.text (String.fromFloat model.bitProbability ++ ")")
                , Html.sup [] [ Html.text (String.fromInt (n - 1)) ]
                , Html.text (" * " ++ String.fromFloat model.bitProbability)
                , Html.text " = "
                , Html.text
                    (String.fromFloat
            ...
            , Html.span []
                ...
                , Html.text " * (1 - "
                , Html.text (String.fromFloat model.bitProbability ++ ")")
                , Html.sup [] [ Html.text (String.fromInt (n - 2)) ]
                , Html.text (" * " ++ String.fromFloat model.bitProbability)
...
singleErrorCorrection model =
    Html.div
        ...
        , Html.p
            ...
            , Html.text (String.fromInt model.hammingModel.n)
            , Html.text " and k = "
            , Html.text (String.fromInt model.hammingModel.k)
            , Html.text " as 2"
            , Html.sup [] [ Html.text (String.fromInt model.hammingModel.k) ]
            , Html.text " = "
            , Html.text (String.fromInt (2 ^ model.hammingModel.k))
...
update msg model =
    case msg of
        ...
        ToggleOddEven ->
            { model | oddeven = modBy 2 (model.oddeven + 1) }

        HammingMsg subAction ->
            let
                updatedHammingModel =
                    HammingPacket.update subAction model.hammingModel
            in
            { model | hammingModel = updatedHammingModel }

        HammingRMsg subAction ->
            let
                updatedHammingRModel =
                    HammingPacket.update subAction model.hammingReceivedModel
            in
        ...
        TransmitPacket ->
            let
                ...
                rModel =
                    model.hammingReceivedModel

                ( bitToBeToggled, nextSeed ) =
                    R.step (R.int 0 transmittedModel.n) rModel.seed

                receivedModel =
                    { transmittedModel | status = "R", bits = transmittedBits bitToBeToggled transmittedModel.bits, seed = nextSeed }
            in
